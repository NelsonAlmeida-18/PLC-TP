...
def p_entrada_ID(p):
    ...
    if varName not in parser.variaveis:
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Erro na linha {parser.linhaDeCodigo}: Variável {varName} não declarada anteriormente"
...
def p_decl_NoAssign(p):
    ...
    if varName not in parser.variaveis:
        ...
        parser.error= f"Erro na linha {parser.linhaDeCodigo}: Variável {varName} já definida."
        parser.exito = False
...
def p_decl_Lista_NoSize(p):
    ...
    if listName not in parser.variaveis:
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Variável com o nome {listName} já definida anteriormente."
        parser.exito = False
...
def p_decl_Matriz_NoSize(p):
    ...
    if matrizName not in parser.variaveis:
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Matriz {matrizName} já definida."
        parser.exito = False
...
def p_decl_alteraVar_Int(p):
    ...
    if varName in parser.variaveis:
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Variável {varName} não definida anteriormente."
        parser.exito = False
...
def p_decl_alteraLista_Int(p):
    ...
    if arrayName in parser.variaveis:
        if indexForReplacement<len(parser.variaveis[arrayName]):
            ...
            parser.error = f"Erro na linha {parser.linhaDeCodigo}: Indice fora de alcance"
            parser.exito = False
    else:
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Variável {arrayName} não definida anteriormente."
        parser.exito = False
...
def p_soma(p):
    ...
    if val1 != None and val2 != None or isinstance(val1,type(val2)):
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Operação impossível entre os tipo de dados {type(val1)}, {type(val2)}!"
        parser.exito=False
...
def p_maisGrande(p):
    ...
    if isinstance(val1, type(val2)):
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Operação inválida entre o tipo de dados {type(val1)}, {type(val2)}"
        parser.exito = False
...
def p_noum(p):
    ...
    if isinstance(val1, int):
        ...
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Operação inválida com o tipo de dados {type(val1)}"
        parser.exito = False
...
def syntatic_sugar(syntaxError):
    ...
    if matches != []:
        parser.error = f"Erro na linha {parser.linhaDeCodigo}: Syntax error na linha {parser.linhaDeCodigo}: Querias dizer {rd.choice(matches)}"
